...
def generate_launch_description():
    package_name = 'four_wheeled_robot_1'

    # 1. Read the URDF file
    xacro_file = os.path.join(
        get_package_share_directory(package_name), 'urdf', 'four_wheeled_robot.urdf.xacro')
    ...
    spawn_robot = Node(
        ...
        arguments=['-topic', 'robot_description', '-entity', 'four_wheeled_robot'],
        output='screen'
    )
    ...
    spawn_cube = Node(
        ...
        executable='spawn_entity.py',
        arguments=['-file',
                   os.path.join(get_package_share_directory(package_name), 'models', 'pickup_cube.sdf'),
                   '-entity', 'pickup_cube',
                   '-x', '0.6', '-y', '0.0', '-z', '0.05'],
        output='screen'
    )
